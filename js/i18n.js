(function (global) {
    ...
    var messages = {
        zh: {
            ...
            exportFormat: '导出格式',
            exportFormatDocx: 'Word 文档 (.docx)',
            exportFormatPdf: 'PDF 文档 (.pdf)',
            exportFormatHelp: '选择导出文件的格式，应用于选中导出与批量导出',
            filenameFormat: '文件名格式',
            filenameFormatHelp: '可用标签: 日期、时间、标题',
            exportTextExcludeSettings: '内容过滤',
            exportTextExcludeEnabled: '过滤平台 UI 文案',
            exportTextExcludeEnabledHelp: '自动去掉 AI代码解释、相关问题、换一批、代码语言 等常见平台组件文字',
            exportTextExcludeCustom: '自定义排除规则',
            exportTextExcludeCustomPlaceholder: '每行一条，例如：\n=整行完全匹配\n展开更多\n/^广告.*/',
            exportTextExcludeCustomHelp: '内置规则已包含常见技术社区 UI。自定义：普通文字在短行（≤48字）内包含即删除；=开头为整行精确匹配；/正则/ 为高级规则',
            ...
            extensionContextInvalid: '扩展已更新，请刷新此页面（F5）后继续使用',
            ...
            errPdfNotLoaded: 'PDF 导出库未加载',
            errHtml2canvasNotLoaded: 'html2canvas 未加载',
            errPdfRenderEmpty: 'PDF 渲染失败，内容为空',
        ...
        en: {
            ...
            exportFormatHelp: 'Applies to selection export and batch export',
            filenameFormat: 'Filename format',
            filenameFormatHelp: 'Available tags: date, time, title',
            exportTextExcludeSettings: 'Content filter',
            exportTextExcludeEnabled: 'Filter platform UI text',
            exportTextExcludeEnabledHelp: 'Removes common UI chrome such as AI code explain, related questions, refresh batch, code language labels',
            exportTextExcludeCustom: 'Custom exclude rules',
            exportTextExcludeCustomPlaceholder: 'One rule per line, e.g.\n=exact full line\nShow more\n/^Ad.*/',
            exportTextExcludeCustomHelp: 'Built-in rules cover common tech blog UI. Custom: plain text removes short lines (≤48 chars) containing it; prefix = for exact line; /regex/ for advanced',
            ...
            extensionContextInvalid: 'Extension updated — please refresh this page (F5) to continue',
            ...
            errPdfNotLoaded: 'PDF export library not loaded',
            errHtml2canvasNotLoaded: 'html2canvas not loaded',
            errPdfRenderEmpty: 'PDF render failed: empty content',
